  ...
  return (
    <div className="shell">
      <header className="header">
        ...
        <div className="header__status" aria-live="polite" aria-label="IPC status">
          {ipc.status === "loading" && <span className="muted small">● connecting…</span>}
          {ipc.status === "success" && <span className="status-ok">● {ipc.data.status}</span>}
          {ipc.status === "error" && <span className="status-error">● error</span>}
          {ipc.status === "idle" && <span className="muted small">● idle</span>}
        </div>
      ...
      {import.meta.env.VITE_M3_RELOAD !== "1" && (
        <section
          className="terminal-core-section terminal-core--full"
      ...
      <details className="diagnostics" open>
        <summary className="diagnostics__summary">Diagnostics</summary>
        <div className="diagnostics-grid">
          <section className="card card--compact" aria-labelledby="ipc-heading">
          ...
          <section className="card card--compact" aria-labelledby="scope-heading">
            ...
            <p className="muted">
              ...
              with session manager, lossless transport, and xterm TerminalView. Workspace/layout
              (M4) and launcher discovery (M5/M6) remain planned.
            </p>
            <ul className="list">
              <li>
                IPC: ping +
              ...
              <li>
                View vs process state orthogonal; renderer reload reattaches to same SessionId
              </li>
            </ul>
          </section>
        </div>
